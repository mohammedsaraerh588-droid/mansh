...
import { Mail, Lock, User, AlertCircle, Stethoscope } from 'lucide-react'
  ...
  if (success) return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg)',padding:'24px'}}>
      <div style={{width:'100%',maxWidth:420,textAlign:'center'}}>
        ...
        <h2 style={{fontSize:24,fontWeight:900,color:'var(--tx1)',marginBottom:10}}>تحقق من بريدك ✉️</h2>
        <p style={{fontSize:15,color:'var(--tx3)',lineHeight:1.85,marginBottom:8}}>أرسلنا رابط التأكيد إلى</p>
        <p style={{fontSize:15,fontWeight:700,color:'var(--tx1)',marginBottom:24}}>{form.email}</p>
        <div className="card" style={{padding:'16px 20px',marginBottom:20,textAlign:'right'}}>
          {['افتح بريدك الإلكتروني','ابحث عن رسالة من منصة تعلّم','اضغط على رابط التأكيد داخل الرسالة','ستدخل المنصة تلقائياً'].map((t,i)=>(
        ...
        <p style={{fontSize:12,color:'var(--tx4)',marginBottom:16}}>لم تصل الرسالة؟ تحقق من مجلد Spam</p>
        <Link href="/auth/login" style={{fontSize:13,color:'var(--alpha-green)',fontWeight:700,textDecoration:'none'}}>العودة لتسجيل الدخول</Link>
      </div>
    </div>
  )
    ...
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg)',padding:'24px'}}>
      <div style={{width:'100%',maxWidth:420}}>
        ...
        <p style={{textAlign:'center',fontSize:11,color:'var(--tx4)',marginTop:16,lineHeight:1.7}}>
          ...
          <Link href="/terms" style={{color:'var(--tx3)'}}>الشروط والأحكام</Link>{' '}و{' '}
