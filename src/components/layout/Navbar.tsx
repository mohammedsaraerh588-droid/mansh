...
export default function Navbar() {
  ...
  const links    = [{ label:'الرئيسية', href:'/', I:Home }, { label:'الدورات', href:'/courses', I:BookOpen }]
  const lc       = (a:boolean) => solid ? (a?'var(--blue)':'var(--txt2)') : (a?'#fff':'rgba(255,255,255,.65)')
  ...
  return (
    ...
    <nav style={{
      ...
      backdropFilter: solid ? 'blur(18px) saturate(180%)' : 'none',
      borderBottom: solid ? '1px solid var(--nav-border)' : 'none',
      boxShadow: solid ? 'var(--s1)' : 'none',
      transition:'all .32s ease',
      padding: solid ? '11px 0' : '18px 0',
    }}>
      <div className="wrap" style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        {/* Logo */}
        <Link href="/" style={{display:'flex',alignItems:'center',gap:10,textDecoration:'none'}}>
          <div style={{width:34,height:34,borderRadius:9,background:'var(--blue)',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:900,fontSize:16,color:'#fff',boxShadow:'var(--sb)',transition:'transform .22s'}}
            onMouseEnter={e=>(e.currentTarget.style.transform='scale(1.08)')}
            onMouseLeave={e=>(e.currentTarget.style.transform='none')}>م</div>
          <span style={{fontSize:16,fontWeight:900,color:solid?'var(--txt1)':'#fff',transition:'color .3s'}}>منصة <span style={{color:'var(--blue)'}}>تعلّم</span></span>
        </Link>
        {/* Desktop links */}
        <div style={{display:'flex',alignItems:'center',gap:2}} className="hidden md:flex">
          {links.map(({label,href,I})=>(
            <Link key={href} href={href} style={{display:'flex',alignItems:'center',gap:6,padding:'7px 13px',borderRadius:9,textDecoration:'none',fontSize:14,fontWeight:600,transition:'all .16s',color:lc(pathname===href),background:pathname===href&&solid?'var(--blue-soft)':'transparent'}}>
              <I size={14}/>{label}
            </Link>
          ))}
        </div>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8}} className="hidden md:flex">
          <button onClick={toggle} className="toggle" style={{justifyContent:dark?'flex-end':'flex-start'}} title="تبديل المظهر">
            <span className="toggle-knob">{dark?<Moon size={10} color="#fff"/>:<Sun size={10} color="#1d4ed8"/>}</span>
          </button>
          {user ? (
            <>
              <Link href={dashLink} style={{display:'flex',alignItems:'center',gap:6,padding:'7px 13px',borderRadius:9,textDecoration:'none',fontSize:13.5,fontWeight:700,color:lc(false),transition:'all .16s'}}
                onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background='var(--blue-soft)';(e.currentTarget as HTMLElement).style.color='var(--blue)'}}
              ...
              <Link href="/profile">
                <div style={{width:34,height:34,borderRadius:'50%',overflow:'hidden',border:'2px solid var(--blue)',cursor:'pointer',transition:'transform .2s'}}
                  onMouseEnter={e=>(e.currentTarget.style.transform='scale(1.08)')}
                  onMouseLeave={e=>(e.currentTarget.style.transform='none')}>
                  {profile?.avatar_url ? <img src={profile.avatar_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/> :
                    <div style={{width:'100%',height:'100%',background:'var(--blue)',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:900,fontSize:13,color:'#fff'}}>{profile?.full_name?.[0]??'أ'}</div>}
                </div>
              </Link>
            </>
          ) : (
