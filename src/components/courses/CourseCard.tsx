  ...
  return (
    <Link href={`/courses/${course.slug}`} style={{textDecoration:'none',display:'block',height:'100%'}}>
      <div className="card card-hover" style={{height:'100%',display:'flex',flexDirection:'column',overflow:'hidden',cursor:'pointer'}}>
        ...
        {/* thumbnail */}
        <div style={{position:'relative',height:188,overflow:'hidden',background:'var(--bg3)',flexShrink:0}}>
          {course.thumbnail_url ? (
            ...
            <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#0d2045,#1e3a6e)'}}>
              <PlayCircle size={44} style={{color:'rgba(96,165,250,.35)'}}/>
            </div>
          )}
          <div style={{position:'absolute',top:10,right:10}}>
            <span className="badge badge-blue" style={{fontSize:10,backdropFilter:'blur(8px)'}}>{getLevelLabel(course.level)}</span>
          </div>
          {course.price===0 && (
            <div style={{position:'absolute',top:10,left:10}}>
              <span className="badge badge-green" style={{fontSize:10}}><Zap size={9}/>مجاني</span>
            </div>
          )}
        ...
        <div style={{padding:'16px',flex:1,display:'flex',flexDirection:'column'}}>
          ...
          {!showProgress && (
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:12,paddingTop:12,borderTop:'1px solid var(--border)'}}>
              ...
              <span style={{fontWeight:900,fontSize:16,color:course.price===0?'var(--ok)':'var(--blue)'}}>{formatPrice(course.price,course.currency)}</span>
            </div>
          )}
          {showProgress && (
            <div style={{marginTop:12}}>
              <div style={{display:'flex',justifyContent:'space-between',fontSize:12,marginBottom:5}}>
                ...
                <span style={{fontWeight:800,color:'var(--blue)'}}>{progress}%</span>
              </div>
